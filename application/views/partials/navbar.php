    ...
    <div class="w-full flex items-center justify-end ms-auto sm:justify-between sm:gap-x-3 sm:order-3">
      ...
      <div class="flex flex-row items-center justify-end gap-2">
        ...
        */
        ...
        <button id="pwa-install-btn" type="button" class="inline-flex items-center justify-center rounded-full border border-cyan-200 bg-cyan-50 p-2 text-cyan-700 opacity-50 cursor-not-allowed dark:border-cyan-700 dark:bg-cyan-800/30 dark:text-cyan-200" aria-label="Install app" aria-disabled="true" title="Install app">
...
<script>
  (function () {
    ...
    function setInstallState(enabled) {
      canInstall = enabled;
      if (enabled) {
        installBtn.classList.remove('opacity-50', 'cursor-not-allowed');
        installBtn.classList.add('hover:bg-cyan-100', 'dark:hover:bg-cyan-700/40');
        ...
        installBtn.classList.remove('hover:bg-cyan-100', 'dark:hover:bg-cyan-700/40');
        installBtn.setAttribute('aria-disabled', 'true');
        installBtn.setAttribute('title', 'Install not available on this page yet');
      }
    }
    ...
    installBtn.addEventListener('click', function () {
      if (!deferredPrompt) {
        if (!canInstall) {
          alert('Install is not available yet. If you are using iPhone, use Share > Add to Home Screen.');
        }
        return;
      }
